...
DySample1::DySample1(G4String giveName)
{

  name = giveName;

  // default parameter values of the sample
  bagThickL      = 5*.00889*cm;
  bagThickR      = 3*.00889*cm;
...
void DySample1::DefineMaterials()
{ 
...
void DySample1::BuildDySample(G4LogicalVolume *logWorld,
                     G4ThreeVector *pos,
                     G4RotationMatrix *rot){

  logicWorld = logWorld;
  SampleRot = rot;
  G4double mid = (bagThickL + bagThickR + sample1Thick)/2;
  G4ThreeVector bagPosL = G4ThreeVector(pos->x()+bagThickL/2-mid,pos->y(),pos->z());
  SamplePos =             G4ThreeVector(pos->x()+sample1Thick/2-mid+bagThickL,pos->y(),pos->z());
  G4ThreeVector bagPosR = G4ThreeVector(pos->x()+bagThickR/2-mid+sample1Thick+bagThickL,pos->y(),pos->z());
...
void DySample1::PrintDySampleParameters(){
  
  G4double SampVol = sample1->GetCubicVolume();
...
void DySample1::SetName(G4String giveName){
    name = giveName;
}
